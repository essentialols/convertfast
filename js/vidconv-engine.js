...
export function getVideoDuration(file) {
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => { URL.revokeObjectURL(url); resolve(0); }, 10000);
    video.src = url;
...
export function getVideoMetadata(file) {
  return new Promise((resolve) => {
    ...
    video.preload = 'metadata';
    const url = URL.createObjectURL(file);
    const defaults = { width: 0, height: 0, duration: 0 };
    const timeout = setTimeout(() => { URL.revokeObjectURL(url); resolve(defaults); }, 10000);
    video.src = url;
    video.onloadedmetadata = () => {
      clearTimeout(timeout);
      URL.revokeObjectURL(url);
