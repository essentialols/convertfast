...
async function extractEpubText(file, onProgress) {
  ...
  if (typeof fflate === "undefined")
    throw new Error("ZIP library not loaded. Please reload the page.");
  let zip;
  try {
    zip = fflate.unzipSync(buf);
  } catch (e) {
    throw new Error(
      "Failed to extract EPUB: file may be corrupted or not a valid ZIP archive.",
    );
  }
  ...
  const containerKey = Object.keys(zip).find(
  ...
  if (orderedFiles.length === 0) {
    orderedFiles = Object.keys(zip)
      .filter(
        (k) =>
          k.match(/\.(x?html?|xml)$/i) && !k.toLowerCase().includes("meta-inf"),
      )
      .sort();
  }
  ...
  for (let i = 0; i < orderedFiles.length; i++) {
    ...
    if (!data) {
      try {
        data = zip[decodeURIComponent(path)];
      } catch {}
    }
    if (!data) continue;
...
async function extractDocxText(file, onProgress) {
  ...
  try {
    ...
    throw new Error(
      "Failed to extract DOCX: file may be corrupted or not a valid DOCX archive.",
    );
  }
  if (onProgress) onProgress(30);

  // Find word/document.xml
  const docKey = Object.keys(zip).find(
    (k) => k.toLowerCase() === "word/document.xml",
  );
...
async function extractMobiText(file, onProgress) {
  ...
  if (buf.length < 78)
    throw new Error("File too small to be a valid MOBI file.");
  ...
  if (recordOffsets.length === 0)
    throw new Error("Invalid MOBI file: no record entries found.");
  ...
  if (rec0Start >= buf.length)
    throw new Error("Invalid MOBI file: record 0 out of bounds.");
  if (rec0End - rec0Start < 14)
    throw new Error("Invalid MOBI file: record 0 header too short.");
  if (rec0Start + 14 > buf.length)
    throw new Error("Invalid MOBI file: record 0 header too short.");
  ...
  if (encryption !== 0) {
    throw new Error(
      "This MOBI file is DRM-protected and cannot be converted. Only unprotected .mobi/.prc files are supported.",
    );
  }

  if (compression !== 1 && compression !== 2) {
    throw new Error(
      "Unsupported MOBI compression type. Only uncompressed and PalmDOC-compressed files are supported.",
    );
  }
  ...
  for (let r = startRec; r < endRec; r++) {
    ...
    const end =
      r + 1 < recordOffsets.length ? recordOffsets[r + 1] : buf.length;
    ...
    if (onProgress)
      onProgress(20 + Math.round(((r - startRec) / (endRec - startRec)) * 40));
  }
